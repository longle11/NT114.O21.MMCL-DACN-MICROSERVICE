        ...
        <div className='d-flex align-items-center'>
            {props.editAttributeTag === field_key_issue ? <div style={{ position: 'relative', padding: 0 }}>
                ...
                <div style={{ position: 'absolute', right: 0, marginTop: 5, zIndex: 99999999 }}>
                    <Button onClick={(e) => {
                        e.stopPropagation()
                        if (field_key_issue === field_key_issue) {
                            dispatch(updateInfoIssue(issueInfo._id, issueInfo.project_id._id, { actual_start: getDate.toISOString() }, getValueOfStringFieldInIssue(issueInfo, field_key_issue) ? dayjs(getValueOfStringFieldInIssue(issueInfo, field_key_issue)).format('DD/MM/YYYY') : "None", getDate.format('DD/MM/YYYY'), userInfo.id, "updated", field_key_issue, projectInfo, userInfo))
                        }
                ...
                <span onDoubleClick={() => {
                ...
                }} style={{ color: '#7A869A' }}>
                    {
                        checkConstraintPermissions(projectInfo, issueInfo, userInfo, 9) ?
                            (date ? dayjs(date).format("DD/MM/YYYY") : "None") : eyeSlashIcon
                    }
                </span>}
        </div>
    )

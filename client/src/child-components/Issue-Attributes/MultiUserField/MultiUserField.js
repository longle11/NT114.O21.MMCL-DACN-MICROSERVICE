...
export default function MultiUserField(props) {
    ...
    const [addMultiUser, setAddMultiUser] = useState(true)
    ...
    return (
        <div className="mt-2">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)' }}>
                {
                    checkConstraintPermissions(projectInfo, issueInfo, userInfo, 9) ? (getValueOfArrayObjectFieldInIssue(issueInfo, field_key_issue)?.length > 0 ? <div className='d-flex'>
                ...
                }
                {
                    checkConstraintPermissions(projectInfo, issueInfo, userInfo, 1) ? <div style={{ width: '100%', marginTop: 5 }}>
                        <button onKeyDown={() => { }} className='text-primary mt-2 mb-2 btn bg-transparent ml-2' style={{ width: 'max-content', fontSize: '12px', margin: '0px', cursor: 'pointer', display: addMultiUser === false ? 'none' : 'block', padding: 0, textAlign: 'left' }} onClick={() => {
                            setAddMultiUser(false)
                        }} >

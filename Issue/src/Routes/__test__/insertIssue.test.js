...
it('returns 401 if authentication is failed', async () => {
    return await request(app)
        .put("/api/issue/comments/insert/1")
        .expect(401)
})
...
it('returns 200 if a comment is successfully inserted in an issue', async () => {
    ...
    const response = await request(app)
        ...
        .expect(201)
    return await request(app)
        .put(`/api/issue/comments/insert/${response.body.data._id.toString()}`)
        .set('Cookie', createFakeCookie())
        .send({commentId: currentComment._id})
        .expect(200)
})

![image](https://github.com/user-attachments/assets/90cd1dee-7505-4822-8d56-b70e1f07b6e5)![image](https://github.com/user-attachments/assets/c88e0794-7ca0-4d45-9c95-353fc3d44da8)![image](https://github.com/user-attachments/assets/ffa3d8e4-11f1-4c59-a851-c26b9ab065db)<h3>Kiến trúc tổng quát của hệ thống microservies</h3>
...
Bước 3: Cấu hình tạo một sub-domain với Record A, trước tiên tạo domain đã đăng ký lên Route53 và ánh xạ các Namepservers từ Route53 vào Namepservers trong tenten
...
Bước 8: Tiến hành cd vào thư mục compose và sử dụng lệnh để xác thực ssl cho domain name sử dụng lệnh “certbot certonly --standalone -d portus.nt533uitjiradev.click  --preferred-challenges http --agree-tos -m ltphilong2001@gmail.com --keep-until-expiring --non-interactive”
<img src="https://github.com/user-attachments/assets/f71a4e8b-46d3-4e28-a44c-a6c92c7d9fcf" alt="database models"/>
...
Bước 7: Tiến hành tạo resource là type là secret để lưu trữ access_key và secret_key để sử dụng cho việc kết nối tới server mino từ service file
...
Bước 8: Nếu muốn backup dữ liệu thì sử dụng lệnh velero restore create <tên backup> --from-backup <tên backup>  --include-namespaces “*” và muốn backup định kỳ theo ngày tự động thì velero schedule create daily-cluster-backup --scheduler"0 0 * * *"  --include-namespace '*'. Sau khi kiểm tra thì trên minio server sẽ có toàn bộ thông tin file backup
